...
# Data: http://nicercode.github.io/intro/preliminaries.html
...
# It's obviously hard to strike a balance, so please pipe up if you're falling behind
...
# BTW: I'll be using "graph", "figure", and "plot" interchangably
...
# Speaking of graphic design, if you think about us all, graphing is the last bastion of creativeness we have.  So enjoy it and develop a style you like.  People who read your papers will know you for it.

# As Rich alluded to last week, tables are sometimes necessary, but always use figure instead if possible -- they really are worth to 1000 words, and can be absorbed much more quickly by someone skimming your paper, and are fun to make!

# Here are some figure that I've made using R over the last few years...

# R plotting is like painting on a canvas.  You can only add things, you can't take them away
...
# save this as what ever you want.
...
# You can see that R does a lot of the basics for you by defulat
...
# You can also plot as lines, which desn't make too much sense here:
...
# 3D plots.  generally have to load an R package.
# Hwoever, two very useful vareities are Image and contour
...
#### Barplots
## Barplots are commonly used in biology, but are not as straightword as you might hope in R. 
# AS rich said last week: there are a couple of basic things that missing in R, probably as a test!
# One is a function to calculate standard error, which you wrote last week
...
# Why did I assign the barplot to a variable called "bp"?
# Type bp
...
# type ?par
# The `par` function helps us design the layout and look and feel of plots.  These are global parameters.
# Meaning that once you set them, they hang around
# This is different to say the plot function.
# However,  plotting functions usually only except some "par" variables.

# E.g., plot excepts "las"
# Other par vaiables can only be set globally,
# For eaxmple, mfrow allows us to do a plot with multiple panels, because each panel then has a plot, the number of panels must be set globally
...
# - You can control the exact dimensions of your plot, e.g., based on journal requirements.
# - You can generate multiple plots in a single R script that get saved to a directory
# - You know that each time to save them, they will be the same as you prescribed

# So, egnerally, unless you're actually building your plots, I'd suggest saving it straight to a folder
...
heights <- seq(min(data$Height), max(data$Height), 1)  # ss <- seq(min(x), max(x), 0.1)

seed_pred <- predict(mod, list(Height = heights), interval = "prediction")
lines(heights, seed_pred[,"lwr"], lty = 2)
lines(heights, seed_pred[,"upr"], lty = 2)

seed_conf <- predict(mod, list(Height = heights), interval = "confidence")
polygon(c(heights, rev(heights)), c(seed_conf[,"lwr"], rev(seed_conf[,"upr"])), col = rgb(0,0,0,0.2), border = NA)
...
pdf("figs/seed_plot.pdf", width=5, height=5)
  plot(Seed.heads ~ Height, data, type="p", axes=FALSE, ann=FALSE, pch=21, col="white", bg="black")
  axis(1)
  axis(2, las=2)
  mtext("Height, cm", side = 1, line = 3)
...
seed_pred <- predict(mod, list(Height = heights), interval = "prediction")
lines(heights, seed_pred[,"fit"]^2, lty = 1)
lines(heights, seed_pred[,"lwr"]^2, lty = 2)
lines(heights, seed_pred[,"upr"]^2, lty = 2)

seed_conf <- predict(mod, list(Height = heights), interval = "confidence")
polygon(c(heights, rev(heights)), c(seed_conf[,"lwr"], rev(seed_conf[,"upr"]))^2, col = rgb(0,0,0,0.2), border = NA)
